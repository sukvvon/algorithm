...
2 초 (추가 시간 없음)	1024 MB (추가 메모리 없음)	15065	7936	6867	54.315%
...
function solution(n, testCases) {
  ...
  function recursion(cnt, testCase, l, r) {
    if (l >= r) {
      return "1 " + (cnt + "");
    } else if (testCase[l] !== testCase[r]) {
      return "0 " + (cnt + "");
    } else {
      return recursion(cnt + 1, testCase, l + 1, r - 1);
    }
  }

  function isPalindrome(testCase) {
    return recursion(1, testCase, 0, testCase.length - 1);
  }

  for (let i = 0; i < n; i++) {
    answer.push(isPalindrome(testCases[i]));
  }
