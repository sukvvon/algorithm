...
1 초 (추가 시간 없음)	1024 MB	5817	2615	1993	46.327%
...
function solution(m, words) {
  const wordsCnts = {};
  const filteredWords = words.filter((v) => v.length >= m);

  for (const word of filteredWords) {
    wordsCnts[word] = (wordsCnts[word] || 0) + 1;
  }

  return [...new Set(filteredWords)]
    .sort((a, b) =>
      wordsCnts[a] === wordsCnts[b]
        ? a.length === b.length
          ? a < b
            ? -1
            : a > b
            ...
            : 0
          : b.length - a.length
        : wordsCnts[b] - wordsCnts[a]
    )
    .join("\n");
}

console.log(solution(m, words));
