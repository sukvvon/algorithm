...
function solution(survey, choices) {
  ...
  for (let i = 0; i < survey.length; i++) {
    const [fir, sec] = [...survey[i]];
    const surveyIndex = hash.findIndex((value) => value[fir] >= 0);

    if (choices[i] - 4 > 0) {
      hash[surveyIndex][sec] += choices[i] - 4;
    } else if (choices[i] - 4 < 0) {
      hash[surveyIndex][fir] += Math.abs(choices[i] - 4);
    }
  }
  ...
  hash.forEach((value) => {
    ...
    if (value[keys[0]] > value[keys[1]]) {
      ...
      const keyAscii = keys.map((value) => value.charCodeAt());
      answer += keyAscii[0] > keyAscii[1] ? keys[1] : keys[0];
    }
  });
